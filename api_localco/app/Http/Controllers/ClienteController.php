...
class ClienteController extends Controller
{
    ...
    {
        return ModelsCliente::with("psfisica", "juridico")->get();
    }

    public function show($id_cliente_pk)
    {
        $cliente = ModelsCliente::with("psfisica", "juridico")->find($id_cliente_pk);
        if (!$cliente) {
    ...
    public function store(Request $request)
    {
        ...
        $cliente = ModelsCliente::create($data);
        return response()->json($cliente, 201);
    }

    public function update(Request $request, $id_cliente_pk)
    {
        $cliente = ModelsCliente::find($id_cliente_pk);
        if (!$cliente) {
            return response()->json(["error" => "Cliente não encontrado"], 404);
        }
